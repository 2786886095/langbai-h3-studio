...
type BenchmarkState = {startedAt:number;executionTarget:'local'|'remote_autodl';resourceSamplingSeconds:number;mode:string;width:number;height:number;duration:number;steps:number;modelFile:string;gpuName:string;driverVersion:string;vramTotal:number;peakVram:number;ramTotal:number;peakRam:number}
type BenchmarkReport = {reportId:string;createdAt:number;executionTarget:string;resourceSamplingSeconds:number;gpuName:string;vramTotalMb:number;peakVramUsedMb:number;peakRamUsedMb:number;generationMode:string;width:number;height:number;durationSeconds:number;elapsedSeconds:number;outcome:string;enabledPlugins:string[]}
...
function App() {
  ...
  const createGenerationJob = async () => {
    ...
    try {
      ...
      const remoteGpu=sshStatus?.running?autoDlProbe?.gpus[0]:undefined
      benchmarkRef.current={startedAt:Date.now(),executionTarget:remoteGpu?'remote_autodl':'local',resourceSamplingSeconds:remoteGpu?0:3,mode:workflowMode,width:localWidth,height:localHeight,duration,steps,modelFile:workflowMode==='ref2va'?'minimax_h3_ref2va_pruned_int8_convrot.safetensors':'minimax_h3_fl2va_pruned_int8_convrot.safetensors',gpuName:remoteGpu?.name||system?.gpu?.name||'',driverVersion:remoteGpu?.driverVersion||system?.gpu?.driverVersion||'',vramTotal:remoteGpu?.vramMib||system?.gpu?.memoryTotalMb||0,peakVram:remoteGpu?0:system?.gpu?.memoryUsedMb||0,ramTotal:remoteGpu?autoDlProbe?.ramTotalMib||0:system?.memoryTotalMb||0,peakRam:remoteGpu?0:system?.memoryUsedMb||0}
  ...
  const exportAnonymousBenchmarks = async () => {
    setBenchmarkExportMessage('')
    const destination=await save({defaultPath:'langbai-h3-匿名基准报告.json',filters:[{name:'JSON 基准报告',extensions:['json']}]})
    if(typeof destination!=='string')return
    try{
      const written=await invoke<string>('benchmark_export_anonymous',{destination})
      setBenchmarkExportMessage(`匿名基准报告已保存：${written}`)
  ...
  const autoDlPanel = autoDlProbe && <div className="autodl-preflight">
    <div className="section-divider"><span>隔离部署预检</span></div>
    <p>Studio 使用独立目录，不覆盖市场模板；同时选择两个模型时会自动共享编码器和 VAE。</p>
    <div className="autodl-variant-row">
      <label><input type="checkbox" checked={autoDlVariants.includes('fl2va')} onChange={event=>setAutoDlVariants(value=>event.target.checked?[...new Set([...value,'fl2va' as const])]:value.filter(item=>item!=='fl2va'))}/> 文字／首尾帧 FL2VA</label>
      <label><input type="checkbox" checked={autoDlVariants.includes('ref2va')} onChange={event=>setAutoDlVariants(value=>event.target.checked?[...new Set([...value,'ref2va' as const])]:value.filter(item=>item!=='ref2va'))}/> 全模态参考 Ref2VA</label>
    </div>
    <button className="primary wide" onClick={preflightAutoDl} disabled={autoDlPlanBusy||autoDlVariants.length===0}>{autoDlPlanBusy?'正在计算空间与共享文件…':'生成隔离部署计划'}</button>
    {autoDlDeployPlan&&<div className="deploy-plan-card"><div><strong>{autoDlDeployPlan.targetPath}</strong><small>部署编号 {autoDlDeployPlan.deploymentId} · 回滚记录 {autoDlDeployPlan.rollbackSupported?'已启用':'未启用'}</small></div><div className="deploy-metrics"><span><b>{(autoDlDeployPlan.requiredBytes/1024/1024/1024).toFixed(1)} GB</b>预计下载</span><span><b>{autoDlDeployPlan.downloadFiles.length}</b>个去重文件</span><span><b>{(autoDlDeployPlan.availableBytes/1024/1024/1024).toFixed(0)} GB</b>远端可用</span></div><small className="deploy-boundary">固定清单、隔离目录、断点续传和逐文件 SHA-256 校验均由 Studio 管理。</small></div>}
    <button className="wide autodl-prepare-button" onClick={prepareAutoDl} disabled={!autoDlDeployPlan||autoDlPrepareBusy}>{autoDlPrepareBusy?'正在创建远端隔离目录…':'确认并创建远端部署目录'}</button>
    {autoDlPrepareResult&&<div className="deploy-journal"><ShieldCheck/><span><strong>远端准备完成</strong><small>{autoDlPrepareResult.progress.map(item=>item.message).join(' · ')} · 脚本 {autoDlPrepareResult.scriptSha256.slice(0,12)}</small></span><button className="journal-refresh" onClick={refreshAutoDlStatus} disabled={autoDlStatusBusy}>{autoDlStatusBusy?'读取中…':'重新读取远端记录'}</button><button className={autoDlRollbackArmed?'journal-rollback armed':'journal-rollback'} onClick={rollbackAutoDl} disabled={autoDlRollbackBusy}>{autoDlRollbackBusy?'回滚中…':autoDlRollbackArmed?'确认回滚':'回滚准备目录'}</button></div>}
    {autoDlPrepareResult&&(()=>{const last=autoDlPrepareResult.progress.at(-1);const data=parseAutoDlDownloadMessage(last?.message);const percent=data.size&&data.downloadedBytes?Math.min(100,data.downloadedBytes/data.size*100):0;return <div className="autodl-download-card"><div className="download-card-head"><span><strong>远端模型下载</strong><small>{data.relativePath||data.file||'尚未开始'} · {last?.stage||'ready'}</small></span><b>{percent.toFixed(1)}%</b></div><div className="progress"><span style={{width:`${percent}%`}}/></div><div className="download-card-stats"><span>{data.downloadedBytes?`${(data.downloadedBytes/1024/1024/1024).toFixed(2)} GB`:'—'} / {data.size?`${(data.size/1024/1024/1024).toFixed(2)} GB`:'—'}</span><span>{data.speedBps?`${(data.speedBps/1024/1024).toFixed(1)} MB/s`:'等待速度'}</span><span>{data.etaSeconds!=null?`约 ${Math.ceil(data.etaSeconds/60)} 分钟`:'等待 ETA'}</span></div><div className="download-card-actions"><button className="primary" onClick={startAutoDlDownload} disabled={autoDlDownloadBusy||autoDlDownloadActive}>{autoDlDownloadBusy?'提交中…':autoDlDownloadActive?'后台下载中':'开始后台下载'}</button><button onClick={cancelAutoDlDownload} disabled={autoDlDownloadBusy||!autoDlDownloadActive}>取消并保留断点</button></div><small>任务在 AutoDL 后台运行，关闭 Studio 不会中止；取消后保留 .part，重新开始即可续传。</small></div>})()}
  </div>
    ...
    <div className={dark ? 'app dark' : 'app'}>
      ...
      {settingsDialog && <div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setSettingsDialog(false)}}>
        <section className="modal" role="dialog" aria-modal="true" aria-labelledby="settings-title">
          ...
          <div className="benchmark-export"><div><strong>导出匿名基准报告</strong><small>只包含硬件、模型文件名、生成规格、耗时和插件；排除提示词、素材、输出路径、ComfyUI 地址、任务 ID 与错误详情。</small></div><button onClick={exportAnonymousBenchmarks} disabled={benchmarkReports.length===0}><Download/> 导出 JSON</button></div>
          {benchmarkExportMessage&&<div className={`probe-message ${benchmarkExportMessage.includes('已保存')?'success':'error'}`}><ShieldCheck/><span><strong>{benchmarkExportMessage}</strong></span></div>}
